...
class UTM_geolocation
{
   uint16   zone;
   float64  easting;
   float64  northing;
private:  // spheroidal parameters?
   bool     bUTM;
   bool     change_reference_elipsoid;    // true  change reference ellipsoid
   float64  CentralMeridian;
   float64  sf;
   float64  er;
   float64  rf;
   float64  A;
   ...
   float64  fe;
   float64  v0, v2, v4 , v6;
private:  // spheroidal calcuations
   // The following are not currently used (not needed for UTM)
   // Some of these may be move to a superclass (coordinate geolocation)?
#  ifdef NOT_YET_NEEDED
   float64 OR1;
#endif
public:
   UTM_geolocation(bool _change_reference_elipsoid = false);
   bool set_UTM(float64 _easting, float64 _northing, uint16 _zone);
   ...
   inline uint16  get_zone()     const { return zone; };
   bool get_degrees(float64 &latitude, float64 &longitude);
   // NYI change_zone(uint16 new_zone)
...
private:
   void update_spheroid_parameters(float64 er ,float64 rf);
};
//______________________________________________________________________________
/*
...
Dim rad, er, , F,  As Double
Dim , pr, en, , , C, U, V, W, , OMO,  As Double
Dim ,  As Double
Dim u0, u2, u4, u6 As Double
Dim  As Double,  As Double, As Double, v6 As Double
...
Public Sub MakeLatLonIntoUTM(ByRef someLat As Double, ByRef someLon As Double)
   ...
   Dim om As Double, S As Double, sinfi As Double, tn As Double
