...
{  //  We now only work with the current data record
   ...
   bool timestep_available      = current_data_rec
     && current_data_rec->contains_time_step(variable,time_query.get_target_date_time());   //040125
...
{  // Since the current data record is not what we are working with,
   ...
   /*170311 moved to CS_UED::Database_file_indexed::locate_data_record
   if (preclude_latest_date_time &&                                              //170311
       (time_query.get_target_date_time() >= preclude_latest_date_time))         //170311
         return 0;                                                               //170311
   */
...
//170311 ,bool substituted_time_query //160331
...
, UED_variable_code for_var_code )                                         const //170323
{
   //170323 assume initialized if (!initialized) initialize();                                               //030607
   assert(initialized); //<- to ensure temporary (in most cases this is redundent, now assumed initialized)
...
, UED_variable_code for_var_code )                                         const //170323
{
   //170323 if (!initialized) initialize();
   assert(initialized); //<- to ensure temporary (in most cases this is redundent, now assumed initialized)
...
/*170311 moved
   if (preclude_latest_date_time)                                                //170310
      latest_date_time = std::min<datetime64>                                    //170310
         (latest_date_time,preclude_latest_date_time);                           //170310
*/
...
{
/*
CORN::Dynamic_array<nat32> unique_codes;                                         //171015
nat32 found_pos=0;                                                               //171015
*/
...
/*
if (!unique_codes.is_in_array(var_def_code,found_pos))                           //171015
   unique_codes.append(var_def_code);
*/

         if (var_def && vi < max_allocation)
         {  // Variable has already been listed, avoid duplication.              //141008
            bool previously_listed = false;                                      //141008
            for (nat16 listed_index = 0; listed_index < /*171015 <= */  vi && !previously_listed;listed_index++)
...
/*
if (unique_codes.is_in_array(var_def_code,found_pos))                           //171015
std::clog << "duplicate " << (int)found_pos << std::endl;
else
   unique_codes.append(var_def_code);
*/
...
Geolocation_record  *Database_file::get_geolocation_record()          stream_IO_
...
//_get_geolocation_record___________________________________________2005-03-07_/
const Geolocation_record  *Database_file::get_geolocation_record_immutable() stream_IO_
{ return (const Geolocation_record  *)get_geolocation_record();
}
//_get_geolocation_record_immutable_________________________________2005-04-29_/
