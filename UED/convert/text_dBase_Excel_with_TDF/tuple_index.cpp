...
{
   if (established) return;                                                      //180107
   established = true;                                                           //180107
...
//______________________________________________________________________________
nat32 Tuple_index::extract_tuples(Bidirectional_list &sorted_tuples) renunciation_
{  nat32 extracted = count;
...
//______________________________________________________________________________
Export_tuple *Tuple_index::find_matching(const Date_time_clad_64 &index_date_time)
{  int32 index = 0;
   ...
   {  case UT_year   : index = index_date_time.get_year()   - first_year; break;
      ...
      default :
         index = -1;                                                             //120610
         cout << "Warning database index time step not implemented" << endl;
      break;
   }
   if (index < 0)
      return 0;
   ...
   if (tuple_level)
      if (!tuples[index])
         tuples[index] = new Export_tuple(index_date_time.get_datetime64());     //170531
   return  (tuple_level)
   ?  tuples[index]
