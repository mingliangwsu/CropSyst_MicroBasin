...
Record_code Tabular_file_with_TDF_convertor::initialize_common()
{
   ...
   record_type_code |=
      data_set_float32_subcode;                                                  //171110
      //171110 data_subcode;
...
// This crashes
      ...
      if (format_column->data_type_labeled.get() == CORN::DT_date)               //141120
      {
         ...
         {  import_column->date_format = new Date_format(units_caption.c_str()); //170531
            import_column->value_datetime.ref_date().prefer_date_format(import_column->date_format); //170826_170521
         }
         /*170531
         import_column->value_datetime.date_format
            .set_string(format_column->get_units_caption());                     //050412
      ...
      if (format_column->data_type_labeled.get() == CORN::DT_time)
      {
         ...
         {  import_column->time_format= new Time_format(units_caption.c_str());  //170521
            import_column->value_datetime.ref_time().prefer_time_format(import_column->time_format);//170826_170521
         }
      }
...
{  nat32  encountered = 0;
   ...
   FOR_EACH_IN(metadata_field,ED_tabular_file_format::Metadata_field,format.metadata_fields_1based,each_metadata_field)
   {
      ...
      nat16 metadata_pos = (metadata_field->pos);  //index                       //171129
      ...
      {  // The width is not specified, presume it is read up to a delimitor
         ...
         nat16 field_width_max = header_row->length() - metadata_pos;
         metadata_field_width = CORN::must_be_less_or_equal_to<nat16>
            (metadata_field_width,field_width_max);
      }
      if (header_row && (header_row->length() >= (metadata_pos+metadata_field_width)))
       {
          std::string metadata_value(*header_row,metadata_pos,metadata_field_width);
...
//if (row_date_time.get_date32() == 2007364)
...
//   tabular_file.goto_first();                                                    //171110
   ...
   {  // Write the caption
      ...
      {  case ED_tabular_file_format::Metadata_field::FD_static_text :
         ...
         {  // get from UED database description record with database_comment_index
            CORN::Text_list comments;   //string
      ...
      if (set_metadata_field_text(header_line[metadata_field->row_num-1],metadata_field->pos/*171129 col_num-1*/,field_value)) //120115
...
{  nat32  exported_col_header_rows = 0;
   ...
   {  SDF_List caption_SDF_list(column->get_caption_SDF());
      ...
      {  // For each column header, take the description (caption) and split the text into words
         ...
         Item_string *caption_fragment =
            dynamic_cast < Item_string *>(caption_SDF_list.pop_at_tail());
         ...
         {  // In this case the caption has more rows than number of rows reserved for column header captions
            ...
            {  std::string caption_fragment_value; remaining_caption_fragment->key_string(caption_fragment_value));
                //180820 std::string caption_fragment_value(remaining_caption_fragment->get_key());
   ...
   for (uint8 CH_row_index = 0; CH_row_index < format.column_headers_delineation.rows; CH_row_index++)
   {
      ...
      {  std::string sheet_name(column->sheet); if (!sheet_name.length()) sheet_name.assign("Sheet 1");
         ...
         if (delimitor&&(curr_pos_num > 1))                                      //110812
   ...
   {  // Output column header units description
      ...
      {  std::string sheet_name(column->sheet); if (!sheet_name.length()) sheet_name.assign("Sheet 1");
         ...
         if (delimitor && curr_pos_num > 1)                                      //090712
            tabular_file->write_delimitor_string(delimitor);
      ...
      {  // Various date variables
         case UT_date :
          { export_column->value_datetime.set_datetime64(tuple_date_time.get_datetime64());
            ...
            //170531 export_column->value_datetime.date_format.set_string(export_column->TFFC_layout.date_time_format); //141120
          ...
          {  if (export_column->TFFC_layout.get_units_code() == UT_date)         //040931
             {
               ...
               export_column->value_datetime.inc_years(parameters.offset_years);  // Options to shift the years in the output (also used to overcome the Y2K problem);   //100303
...
{  nat32 export_count = 0;
   ...
   */
   ...
   {  CORN::SDF_List UED_filenames(parameters.UED_filenames.c_str(),false);      //180308
...
//_2017-11-25___________________________________________________________________
/*171125
...
//_2017-11-25____________________________________________________export_tuples_/
