...
{}
//__1999-01-08__________________________________Data_record_header:constructor_/
...
//,UED_num_values_in_data_record(0)                                              //140612
{}
//_1999-11-21___________________________________Data_record_header:constructor_/
Data_record_header::Data_record_header
...
//_1999-01-08___________________________________Data_record_header:constructor_/
Data_record_header* Data_record_header::clone()                            const
{  return new Data_record_header(*this);
}
//_2014-01-18____________________________________________________________clone_/
...
//_1999-01-08______________________________________________________identify_IO_/
...
{  //This method must only be called by derived Binary_data_record
   Date_time_clad_64 smart_timestamp(timestamp);                                 //030714
   if (IO_num_values)
   {  nat16 num_values = get_num_values();                                       //000517
      parent_database.write_nat16(num_values);
   }
   if (IO_year)
      parent_database.write_nat16(smart_timestamp.get_year()); // For year, we the year;
   else
      if (IO_date && IO_time)                                                    //030714
         parent_database.write_float64(timestamp);                               //030714
      else                                                                       //030714
      {  if (IO_date)
           parent_database.write_nat32(smart_timestamp.get_date32());            //030714
         if (IO_time)
            parent_database.write_nat32(smart_timestamp.get_HHMMSS());           //990336
      }
...
//__________________________________________________________________write_body_/
...
//_1997-11-03_________________________________________compute_record_body_size_/
nat32 /*180308 nat16*/ UED_num_values_in_data_record = 0;
...
{  identify_IO() ;
   UED_num_values_in_data_record = (IO_num_values)                               //000517
     ...
     : (nat16)1;
   if (IO_year)
   {  Date_time_clad_64 smart_timestamp(0);                                      //030714
      Year year = parent_database.read_nat16();
      smart_timestamp.set_YD(year,1); // store single years in date
      timestamp = smart_timestamp.get_datetime64();                              //030714
   } else
   if (IO_date && IO_time)                                                       //030714
      timestamp = parent_database.read_float64();                                //030714
   else                                                                          //030714
   {  Date_time_clad_64 smart_timestamp(0);                                      //030714
      if (IO_date) smart_timestamp.set_date32(parent_database.read_nat32());     //030714
      if (IO_time) smart_timestamp.set_HHMMSS(parent_database.read_nat32());     //030714
      timestamp = smart_timestamp.get_datetime64();                              //030714
   }                                                                             //030714
...
//_2000-07-05________________________________________________________read_body_/
datetime64 Data_record_header::get_final_time_series_date()
{
   Date_time_clad_64 smart_timestamp(timestamp);                                 //030714
   Date_time_clad_64 final_time_series_date(smart_timestamp.get_date32());       //170424_030714
   final_time_series_date.set_datetime64(timestamp);                             //030714
   Year year = smart_timestamp.get_year();
   ...
   { switch (timestep_units_code)
     { case UT_year :
       ...
       {  CORN::Month  month;                                                    //990301
          CORN::DOM    dom;                                                      //990301
          smart_timestamp.gregorian(year,month,dom);                             //990301
...
datetime64 Data_record_header::get_final_time_series_time()
{
   ...
   final_time_series_time.set_datetime64(smart_timestamp.get_time64());          //030714
   nat16 num_values = get_num_values();
   switch (timestep_units_code)
   {   case UT_hour   : {  smart_timestamp.inc_hours(num_values);  }  break;     //000517
          // num_values should be 60
       case UT_minute : {  smart_timestamp.inc_minutes(num_values);}  break;     //990306
       case UT_second : {  smart_timestamp.inc_seconds(num_values);}  break;     //990306
   }
...
{  Date_time_clad_64 smart_check_date_time(check_date_time);                     //030714
   Date_time_clad_64 smart_timestamp(timestamp);                                 //030714
  ...
  // This selection should work for either a data set or a single value
   ...
   {  if ((smart_check_date_time.get_date32() < smart_timestamp.get_date32())    //030714
       || (smart_check_date_time.get_date32() > (int32)get_final_time_series_date()))
        return false;  // The date is out of range
   } else //we don't check the year (it is built into the date)
   if (has_year_time_stamp())
   {  Year rec_time_stamp_year = smart_timestamp.get_year();                     //030714
...
Binary_data_record_clad::Binary_data_record_clad(const Binary_data_record_clad& to_be_copied)
...
//_1998-10-02___________________________________________________________________
nat16 Binary_data_record_cowl::count_data_values/*180211 data_values_get_count*/() const
{  return
   is_data_scalar()
   ? (int16)1
...
{  Date_time_clad_64 date_time_index(date_time_idx);                             //030714
   ...
   { case UT_year :
      {  // In this case we simply index by year
         index =(int16)(date_time_index.get_year()-smart_timestamp.get_year());  //030714
      } break;
      case UT_biannum:                                                           //030714
      {  index =(int16)((date_time_index.get_year()-smart_timestamp.get_year())*2 //030714
          + (date_time_index.get_month() >= 7 ? 1 : 0));
      } break;
      case UT_quarter :
      {  index = (int16)((date_time_index.get_year()-smart_timestamp.get_year())*4);  //030714
         int quarter = 0;
      ...
      case UT_month :
      {
         index = (int16)((date_time_index.get_year() - smart_timestamp.get_year()) * 12); //030714
         index += (int16)(date_time_index.get_month() - 1);
      } break;
      case UT_week :
      { index = (int16)((date_time_index.get_year() - smart_timestamp.get_year()) * 52); //030714
        index += (int16)(date_time_index.get_DOY() * 52 / 366);
      } break;
      case UT_day     :
      { // num_values should be 365 or 366 for leap years.
         Date_clad_32 ts_date(smart_timestamp.get_date32());                     //030714
...
{  nat32  attribute_count = D_attributes.get_count();
   CORN::Quality_code quality_code = attribute_count ? D_attributes.get(index) : attribute_homogeneous; //180326
