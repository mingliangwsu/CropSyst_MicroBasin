...
*/
   ...
   edit_source_URL->Text = WebUpdate_VX->URL;
};
//---------------------------------------------------------------------------
...
void __fastcall Tform_install_update::WebUpdate_VXGetFileList(TObject *Sender,TStringList *List)
{
#ifdef NYI
...
//---------------------------------------------------------------------------
void __fastcall Tform_install_update::WebUpdate_VXProgressCancel(TObject *Sender,bool &Cancel)
{
...
void __fastcall Tform_install_update::button_updateClick(TObject *Sender)
{
   ...
   /*180103 Now allways install in program files
...
void __fastcall Tform_install_update::button_register_file_typesClick(TObject *Sender)
{
   ...
   */
   nat8 setup_version_index = radio_group_version->ItemIndex; // 0 is currently V4
   nat8 setup_version_number = setup_version_index + 4;
   registry_subkey += ('4' + setup_version_index);

   CORN::Registry *registry_CropSyst_Suite_X = new CORN::Registry(registry_subkey.c_str());
   try
   {
      if (setup_version_number <= 4)
      {
...
void Tform_install_update::add_menus(bool all_users)
{
   ...
   nat8 setup_version_index = radio_group_version->ItemIndex; // 0 is currently V4
   nat8 setup_version = setup_version_index + 4;
...
{
   WebUpdate_VX->URL = edit_source_URL->Text;
}
//---------------------------------------------------------------------------
void __fastcall Tform_install_update::radio_group_versionClick(TObject *Sender)
{
   edit_source_URL->Text = L"http://modeling.bsyse.wsu.edu/rnelson/CS_suite_5/installation/update_control_CropSyst_complete.inf";
}
//---------------------------------------------------------------------------
