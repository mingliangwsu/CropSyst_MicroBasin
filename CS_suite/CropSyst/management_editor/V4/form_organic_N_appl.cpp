...
#else
#define org_N_source_cowl_or_labeled  org_N_source_cowl
#define org_N_appl_method_cowl org_N_appl_method_cowl
#endif
*/
...
/*150403 Now obsolete, replaced with biomatter application

   // Organic manure page
   org_N_amount_edit          ->bind_to(&(operation->v_org_N)                       ,0);
   org_N_decomp_time_63_edit  ->bind_to(&(operation->v_decomposition_time_63)       ,0);
   org_N_decomp_time_50_edit  ->bind_to(&(operation->v_decomposition_time_50)       ,0);
   org_N_decomp_time_check    ->bind_to(&(operation->decomposition_time_calculated) ,0);
   manure_type_radiogroup     ->bind_to(&(operation->org_N_source_cowl_or_labeled)  ,0);
   NH3_ammonia_edit           ->bind_to(&(operation->v_NH3_N)                       ,0);
   // Volatilization page
   NH3_volatil_loss_edit      ->bind_to(&(operation->v_long_term_org_N_volatilization_loss_percent),0);
   NH3_volatil_loss_check     ->bind_to(&(operation->org_N_volatilization_calculated)              ,0);
   org_appl_method_radiogroup ->bind_to(&(operation->org_N_appl_method_cowl_or_labeled)            ,0);
   orgN_form_radiogroup       ->bind_to(&(operation->solid_liquid_form_labeled)                    ,0);
   edit_carbon_fraction       ->bind_to(&(operation->v_carbon_fraction)                            ,0);
   ...
   est_org_drymatter_out->bind_to(&(est_org_drymatter),2,0);
...
/*150403 Now obsolete, replaced with biomatter application
   operation->decomposition_time_50 = convert_decomposition_time_63_to_50
      (manure_decomp_time_coef[operation->org_N_source_cowl_or_labeled.get() ]);
