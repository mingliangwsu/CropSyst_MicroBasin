...
Association_list_obsolete::Association_list_obsolete(bool _unique_keys)
: Bidirectional_list()
{
...
{  // First see if it is a normal association.
   Association *result =
      ...
      (Association_list_obsolete::find_cstr(search_key,case_sensitive));
   // If we didn't find the association see if there is an alias
   if (!result)
   {  Alias_obsolete *matching_alias = (Alias_obsolete *)aliases.find_cstr(search_key,case_sensitive);
      if (matching_alias)
      {  const char *actual = matching_alias->get_actual();                      //030117
         result =
            dynamic_cast<Association *>                                          //140716
            (Association_list_obsolete::find_cstr(actual,case_sensitive));
      }
   }
   return result;
}
...
{  Alias_obsolete *new_alias = new Alias_obsolete(actual_variable,alias_variable);
   aliases.append(new_alias);
}
//_2001-12-09___________________________________________________________________
Alias_obsolete::Alias_obsolete(const char *i_actual,const char *i_alias)
: alias  (0)
, actual (0)
