...
bool Binary_record_file_dynamic_indexed::initialize()
{
   ...
   std::ios_base::openmode /*170117 int */ mode = binary_file->get_open_mode();  //170117
   index_node = 0;                                                               //030710
...
void Binary_record_file_dynamic_indexed::dumpX(std::ostream &txt_strm,bool full_record_dump)
...
void Binary_record_file_dynamic_indexed::write_YAML(std::ostream &YAML_strm,nat16 indent_level,bool full_record_dump)
{  FOR_EACH_IN(dmp_index_node,Binary_record_index_node_dynamic,index,each_node)  //030709
   {  if (full_record_dump)
         dmp_index_node->get(*this);
      // otherwise so far we have only read record headers
