...
(Bidirectional_list &_tabular_data_columns,const char *_primary_section
...
(const std::string &current_sheet_name,nat16 cell_column_1base)
{  nat16 col_index = 1;
...
{  bool expected = CORN::Data_record::expect_structure(for_write);               //161023
   ...
   {  // depending on the data type expect to value_*
      ...
      {  case CORN::DT_bool         : entry = expect_bool        (TC_layout_key.c_str()/*180820 tab_data_col->TC_layout.get_key()*/,tab_data_col->value_bool);                      break;
         case CORN::DT_string       : entry = expect_string      (TC_layout_key.c_str()/*tab_data_col->TC_layout.get_key()*/,tab_data_col->value_string,tab_data_col->TC_layout.get_field_width()/*width*/); break;
         case CORN::DT_int16        : entry = expect_int16       (TC_layout_key.c_str()/*tab_data_col->TC_layout.get_key()*/,tab_data_col->value_int16);                     break;
         case CORN::DT_int32        : entry = expect_int32       (TC_layout_key.c_str()/*tab_data_col->TC_layout.get_key()*/,tab_data_col->value_int32);                     break;

     // Currently I don't have buffers for nat16 and nat32 so simply using int16 and int32
         case CORN::DT_nat16        : entry = expect_int16       (TC_layout_key.c_str()/*tab_data_col->TC_layout.get_key()*/,tab_data_col->value_int16);                     break;
         case CORN::DT_nat32        : entry = expect_int32       (TC_layout_key.c_str()/*tab_data_col->TC_layout.get_key()*/,tab_data_col->value_int32);                     break;

         case CORN::DT_float32      : entry = expect_float32     (TC_layout_key.c_str()/*tab_data_col->TC_layout.get_key()*/,tab_data_col->value_float32);                   break;
         case CORN::DT_float64      : entry = expect_float64     (TC_layout_key.c_str()/*tab_data_col->TC_layout.get_key()*/,tab_data_col->value_float64);                   break;
         case CORN::DT_date         :
            entry = expect_datetime64  (TC_layout_key.c_str()/*tab_data_col->TC_layout.get_key()*/,tab_data_col->value_datetime,true);                                   //120331
         break;
//         case CORN::DT_time         : entry = expect_int32  (TC_layout_key,tab_data_col->value_int32);                  break; //170525
         ...
         case CORN::DT_time         : entry = expect_datetime64  (TC_layout_key.c_str()/*tab_data_col->TC_layout.get_key()*/,tab_data_col->value_datetime,true);                  break;
         //case CORN::DT_datetime     : entry = expect_float64  (TC_layout_key,tab_data_col->value_float64);                  break; //170525120331
         case CORN::DT_datetime     : entry = expect_datetime64  (TC_layout_key.c_str()/*tab_data_col->TC_layout.get_key()*/,tab_data_col->value_datetime,true);                  break; //120331
         default              :                      entry = expect_string      (TC_layout_key.c_str()/*tab_data_col->TC_layout.get_key()*/,tab_data_col->value_string,tab_data_col->TC_layout.get_field_width()/*width*/); break;
      }
      if (entry)                                                                 //050516
