...
private:
   void SigFig(STD_NS ostream& os, double value, int nsig) const;
   void DecFig(STD_NS ostream& os, double value, int ndec, bool force_decimal=false)
      const;
   void IntegerPrint(STD_NS ostream& os, double value) const;
   void Scientific(STD_NS ostream& os, double value, int nsig) const;

   void FormatOut(STD_NS ostream& os, double value) const;

   void StringPrint(STD_NS ostream& os, const char* value, int size) const;

   void ErrorOut(STD_NS ostream& os) const;
};
...
class OstreamWithFormat
{
   ...
   const Format& f;
public:
   OstreamWithFormat(STD_NS ostream& osx, const Format& fx)
      : os(osx), f(fx) {}
      
   OstreamWithFormat(STD_NS ostreamWithFormat const& owf)
      : os(owf.os), f(owf.f) {}
   ...
   OstreamWithFormat& operator<< (STD_NS ostreamWithFormat& (*_f)(STD_NS ostreamWithFormat&))
      { _f(*this); return *this; }
      
   const Format& format() const { return f; }
   
   friend OstreamWithFormat operator<<(STD_NS ostreamWithFormat& osx, const Format& fx)
      { return OstreamWithFormat(osx.os, fx); }

   friend OstreamWithFormat& endl(STD_NS ostreamWithFormat&);
   friend OstreamWithFormat& ends(STD_NS ostreamWithFormat&);
   friend OstreamWithFormat& flush(STD_NS ostreamWithFormat&);
   
};

inline OstreamWithFormat operator<<(STD_NS ostream& osx, const Format& fx)
