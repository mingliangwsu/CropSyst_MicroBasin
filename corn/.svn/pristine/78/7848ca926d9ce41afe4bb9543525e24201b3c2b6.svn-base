...
void SDF_List::update()
{
      ...
      {  case '\"'  :
         { if (!in_quote)
            ...
            {  if (ci1 == '\"')
               ...
               {  // we reached the end of the string
                  append(new Item_string /*180115 Item*/(item_value.c_str()));
                  item_value.assign("");
         ...
         { if (in_quote )
            ...
            {  bool delimitor = (space_is_a_delimiter && (ci == ' ')) || ci == ','; //090901
               ...
               {  append(new Item_string /*180115 Item*/(item_value.c_str()));
                  //171125 append(new SDF_List::Item(item_value));
                  item_value.assign("");
                  started_value = false;
                  in_unquoted_string = false;                                    //041003
               } else
                  ...
                  {  // we encountered a successive comma, so we append an empty string
                     append(new Item_string /*180115 SDF_List::Item*/(""));
                     in_unquoted_string = false;                                 //041003
                 }
   ...
   if (item_value.length()) append
      (new Item_string /*180115 Item*/(item_value.c_str()));                                            //171125
   updated = true;
}
//______________________________________________________________________________
const char *SDF_List::get_cstr() const
{  SDF_list_string.assign("");
   FOR_EACH(item,Item_string /*180115 SDF_List::Item*/,each_item)
     append_SDF_to(*item,SDF_list_string,true);                                  //171125_130919
     //171125 append_SDF_to(item->value,SDF_list_string,true);                            //130919
