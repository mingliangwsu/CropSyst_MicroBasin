...
Binary_record *Excel_workbook::instanciate_record(Record_code code) const
{
...
//______________________________________________________________________________
Workbook::Sheet *Excel_workbook::provide_sheet(const std::string &sheet_name)  // was get_sheet
{  if (recent_sheet && 
       recent_sheet->is_key_string(sheet_name) //180820 
      //180820 (strcmp(sheet_name.c_str(),recent_sheet->get_key()) == 0)
      )   //100110
      return recent_sheet;                                                          //100110
   Excel_worksheet *sheet = dynamic_cast<Excel_worksheet *> /*130401 (Excel_worksheet *)*/(worksheets.find_string(sheet_name/*130401 .c_str()*/));
   if (!sheet)
   {  // If the named sheet doesn't exist, create it.
      sheet = new Excel_worksheet(sheet_name,BIFF_rec_BOUNDSHEET_common::WORKSHEET,format);
...
//______________________________________________________________________________
Excel_worksheet *Excel_workbook::take_sheet(Excel_worksheet *sheet_to_add)
{  worksheets.append(sheet_to_add);
   recent_sheet = sheet_to_add;                                                  //100110
   return sheet_to_add;
};
//______________________________________________________________________________
void Excel_workbook::commit()
{
...
//100111                      ;sheet = (Excel_worksheet *)worksheets.move_to_next())
         FOR_EACH_IN(sheet,Excel_worksheet,worksheets,each_worksheet)               //100111
   ...
   while (Excel_worksheet *sheet = dynamic_cast<Excel_worksheet *>(worksheets.pop_at_head()))
      sheet->commit(*this);
...
//______________________________________________________________________________
const std::string &Excel_workbook::get_name(std::string &return_name)  const
{
