...
//_2018-02-01_________________________________________________________is_valid_/
...
//_2012-02-14__________________________________________________________________/
Directory_entry_name *Directory_entry_name_abstract::render_with_respect_to      //180104
(
// const Directory_entry_name &old_DEN
  const Directory_entry_name &respect_DEN)                            rendition_
{
   ...
   /*old_DEN     .*/append_path_list_to(    old_path,include_path_designated); // omitting filename.ext
   CORN::Text_list respect_path;
   respect_DEN .append_path_list_to(respect_path,include_path_designated); // omitting filename.ext
   ...
   {  for (nat8 o = 0 ; o < dir_match_end_in_old; o++)
      ...
      for (nat8 o = dir_match_end_in_old+1; o < old_dir_count; o++)
      {  Item_wstring *keep_o = dynamic_cast <Item_wstring *>(old_path.pop_at_head());
         resolved.take(keep_o);
      }
   }
   if (resolved.count())
   {
      std::wstring name_with_ext;
      resolved.add_wstring(/*old_DEN.*/get_name_with_extension(name_with_ext));
      rendered = new Directory_entry_name_concrete(resolved);
   }
   return rendered;
}
//_2018-01-03_2007-01-06_______________________________render_with_respect_to__/
