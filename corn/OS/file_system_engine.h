...
interface_ File_system_engine
{
 public: // Methods that change the file system structure
   virtual bool create_directory                                                 //121222
      ...
      //180321 use provide_directory for this case ,bool create_parents_as_needed = true
      )                            const=0;
   ...
   */
   ...
   virtual Directory_entry_name *render_resolved_absolute_DEN
      ...
      /**<
         ...
         $variable: (conceptual) specified environment variable. The function will return 0 if not defined.
         ~CONTEXT: Search stops at users home directory (if encountered otherwise continues to root)
         -CONTEXT: (conceptual probably not needed) Search continues to root (does not stop at users home directory).

            Search the path of the with_respect_to_directory_name
            - search for the first DEN following the :
               I.e.  $CONTEXT:/a/b/c/f.ext will search for directory 'a'
               and replace $CONTEXT: with the parent path of a.
   ...
   virtual Directory_entry_name *find_qualified_name_in_path
      ...
      ,Path_inclusion include_path_directories_in_search
