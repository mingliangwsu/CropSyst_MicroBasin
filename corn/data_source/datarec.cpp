...
VV_date_entry *Data_record::expect_date   (const char *_key ,modifiable_  CORN::Date_clad_32 &_value, bool formatted, Phrase_label _annotation1,Phrase_label _annotation2)
{  Association_list &section_entries = get_current_section()->entries;
...
VV_datetime64_entry * Data_record::expect_datetime64(const char *_key ,modifiable_  CORN::Date_time_clad_64 & _value, bool formatted, Phrase_label _annotation1,Phrase_label _annotation2)
{  Association_list &section_entries = get_current_section()->entries;
...
{  if (!enumerate_variable)
       set_current_section(_variable_or_section_label);
   for (nat16 i = one_based; i <= max_index; i++)
      expect_int32(enum_variable(i,_variable_or_section_label,enumerate_variable),array[i]);
}
//_2003-08-01__1998-12-12_______________________________________________________
...
//______________________________________________________________________________
bool VV_enumerated_section::setup_structure(Data_record &data_rec,bool for_write) modification_
{  data_rec.structure_defined =false;                                            //120314
   std::string current_section_key(data_rec.get_current_section()->get_label()/*180821 get_section_label()*/); //040909
...
bool Data_record::expect_structure(bool /*unused for_write*/)
...
//_1998-04-05___________________________________________________________________
bool Data_record::Units_label_item::is_key_string(const std::string &key) affirmation_  //180820
...
bool Data_record::notify_indexer(CORN::Indexer &indexer)
{
   ...
   {  const char *section_label=section->get_label/*180821get_section_label*/().c_str(); //180820 get_key();
      ...
      {  // lookup field in actual fields:
         const char *entry_label = entry->get_variable().c_str(); //180820 get_key();
...
//std::clog << "debug resolved_DEN:" <<  entry_DEN.c_str() << std::endl;
...
//_2016-10-23________________________________________________________get_start_/
} // namespace CORN
