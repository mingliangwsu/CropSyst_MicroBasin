...
{  nat16 field_pos = 0;
   ...
   {  //for each entry in current section:
      ...
      {  // lookup field in actual fields:
         ...
         const std::string &entry_label =    entry->get_variable();                                               //180820
...
//NYI       case VV_nat8     : field_type = dB_number   ; /*length =   3;*/   break;
         ...
         Field_item *field = new Field_item(entry_label.c_str(),field_type ,field_length,field_decimals,field_pos); //180820_030117
...
*/
   ...
   {  // Fill record buffer
      ...
      {     //for each entry in current section
            FOR_EACH_IN(entry,VV_abstract_entry,section->entries,each_entry)     //030709
            {  // lookup field in actual fields:
               const char *entry_label = entry->get_variable().c_str()/*180820 get_key()*/;                       //030117
               ...
               if (field)
               {
                  ...
                  */
                  ...
                  /*170525 This was to append gregorian date comment it is superfluous
...
{  return key == Field_item_name_return;
}
//______________________________________________________________________________
/*180820 Probably need something like get_variable, or get_name
...
bool Dynamic_string_indexed_dBase_data_source::Index_item::is_key_string(const std::string &key)affirmation_  //180820
...
{  nat32 pos = (nat32)std::string::npos;
   FOR_EACH_IN(field,Field_item,actual_fields,each_field)                        //030709
   {  std::string trimed_name(field->get_name()/*180821 get_key()*/);          //040909
      //180820 std::string trimed_name(field->get_variable()/*180821 get_key()*/);          //040909
   ...
   if (current_record)
      if (position_record_pointer(current_record))
      {
         ...
         FOR_EACH_IN(section,VV_Section,data_rec.sections,each_section)          //030709
         {
            FOR_EACH_IN(entry,VV_abstract_entry,section->entries,each_entry)     //030709
            {  // lookup field in actual fields:
               const char *entry_label = entry->get_variable().c_str()/*180820get_key()*/;                       //030117
...
/*170928 caller now simply uses directly the field names
