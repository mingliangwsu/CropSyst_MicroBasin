...
#define RDB_STRING__
#else
// RLN Roger Nelson  When compiled under BC5 with no name space there is a conflict with String
// using :: removes the abiguity.
// The RDB_STRING__ macro adds :: when compiled with no name space  and leaves it of when namespaces are enabled
#define RDB_STRING__ ::
#endif
...
public:
   ...
   void swap(RDB_STRING__ String&);  // RLN
   ...
   friend void swap(RDB_STRING__ String& A, RDB_STRING__ String& B); // RLN

   friend STD_NS istream& operator>>(istream& is, RDB_STRING__ String& str); // RLN
   friend STD_NS ostream& operator<<(STD_NS ostream& os, const String& str);
   friend STD_NS istream& getline(istream& is, RDB_STRING__  String& str, char delim = '\n');  //RLN
...
public:
   ...
   CharSeq(const RDB_STRING__  String& str); // RLN
   CharSeq(const RDB_STRING__  String& str, uint pos, uint n);
   uint size() const;
   uint capacity() const;
...
inline void swap(RDB_STRING__  String& A, RDB_STRING__  String& B) { A.swap(B); }
...
// This must be in the header file (Precomiling headers will complain)
// A new StringPackageInitialise object is needed for each file where String is used
// This is to make sure the String class is properly initialised before any
// global String is declared.  The String Package Initialise constructor must
// have been run before any global String object is constructed.
