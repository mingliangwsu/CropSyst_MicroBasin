...
public:
    ...
    void AllocateMem();
    void InitAllNovalue();
    void readAsciiGridFile(std::string filename);
...
template <class Grid_Type> void EsriGridClass<Grid_Type>::readAsciiGridFile(std::string filename)
//______________________________________________________________________________
{
    FILE *fgrid;
    ...
    else fscanf(fgrid,"%s %lf",ctmp,&novalue);
    //fgets(ctmp,50,fgrid);
    AllocateMem();
    for (int i = 0; i < nrows; i++) {
        for (int j = 0; j < ncols; j++) {
            if (bInt) fscanf(fgrid,"%d",&value[i][j]);
            else fscanf(fgrid,"%lf",&value[i][j]);
