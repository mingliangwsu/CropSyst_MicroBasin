...
delete this
   ...
   #ifdef SALINITY
   if (components.sal_ID)
   {  Database_data_source salinity_table(LADSS_database_schema table("SALINITY"),"SAL_ID" ,std::ios_base::in  database_session);
      salinity_table.where_long("SAL_ID",components.sal_ID);
      salinity_table.get(crop);
   }
   #endif
   #ifdef CO2_CHANGE
   if (components.CO2_ID)
   {  Database_data_source CO2_table(LADSS_database_schema table("CO2"),"CO2_ID" ,std::ios_base::in  database_session);
      CO2_table.where_long("CO2_ID",components.CO2_ID);
      CO2_table.get(crop);
   }
   #endif
...
bool LADSS_rotation_components_data_source::get(CropSyst::Rotation_parameters &rotation_params)
{
   ...
   {  switch (rot_comps_DR.land_use_type_labeled.get())
      {  case CROP_LAND_USE :
            rotation_params.planting_events.append(dynamic_cast<CORN::Item *>(create_sowing_event(rot_comps_DR)));
         break;
...
{  CropSyst::Sowing_event *sow_event = new CropSyst::Sowing_event;
   ...
   sow_event->begin_sync.set_date32(rot_comp_DR.event_date);                     //170525
      char ID_cstr[10];
...
{  bool expected = Common_event::setup_structure(*this,for_write);
   ...
   expect_int32("EVENT_DATE",begin_sync.mod_date32());                           //170525
