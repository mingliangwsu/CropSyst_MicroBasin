...
float64 Soil_hydrology_abstract::calc_water_potential
...
{  float64 AW_to_depth = 0;
   ...
   //170626brc //float64 AW_depth_layer_last= AW_volumetric_last * remaining_depth;
...
{  // this used to be done by soil_freezing
   ...
   {  float64 old_liquid_water_WC = get_liquid_water_content_volumetric(layer);  //080128
      ...
      //170217unused float64 fract_frozen =
...
float64 Soil_hydrology_abstract::initialize_moisture
(nat8 layer
,Moisture_characterization moisture
,float64 WC_initial)                                               modification_
{
   /* 170530brc
   if (CORN::is_approximately<float64>(WC_initial,0.0,0.000001))
      moisture = average;
   */
   if (CORN::is_approximately<float64>(WC_initial,0.0,0.000001) && !moisture)   //170530brc
      moisture = average;
   switch (moisture)
   {
      case dry: WC_initial
...
RENDER_INSPECTORS_DEFINITION(Soil_hydrology_abstract)
{
   ...
   inspectors.append(new CS::Inspector_scalar(depth_profile_water[1],UC_meter,*context,"water_depth/profile/LEQ",value_statistic,CSVP_UNKNOWN)); //170319
   //170319 inspect the value for the profile, the tally will handle
   // initial and final
   /*170319
   // for start of period
   inspectors.append(new CS::Inspector_scalar(depth_profile_water[1],UC_meter,*context,"water_depth/profile/LEQ",initial_statistic,CSVP_UNKNOWN));
   //for end of period
   inspectors.append(new CS::Inspector_scalar(depth_profile_water[1],UC_meter,*context,"water_depth/profile/LEQ",final_statistic,CSVP_UNKNOWN)); // NYI actually final_statistic
   */
