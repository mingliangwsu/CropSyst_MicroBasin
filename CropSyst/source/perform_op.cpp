...
//180822 #  include "common/simulation/simulation.h"
         ...
         if (meteorology.has_atmospheric_CO2_change())                           //151201
         {
            CO2_response = new
               CropSyst::Crop_CO2_response
                  ...
                  //180805, ET_ref->ref_aerodynamic_resistance_ref_plants()
                  #else
...
#error reached
            ...
            {
               Orchard_interface *orchard = provide_orchard_interception_model(*crop_params);   //071117
               ...
               default :
               {
                  ...
                  /*180417
                  ,scenario_control_and_model_options.dry_soil_root_activity_coef //170518
                  ,scenario_control_and_model_options.saturated_soil_root_activity_coef //170518
                  */
                  ,today_raw                                                     //170525
...
//180101unused                if (crop_at_harvest      )crop_at_harvest      ->know_residues(organic_matter_residues_profile_kg_m2->mod_residues());
               if (crop_at_end_season   )crop_at_end_season   ->know_residues(organic_matter_residues_profile_kg_m2->mod_residues());
               //180101 if (crop_terminated      )crop_terminated      ->know_residues(organic_matter_residues_profile_kg_m2->mod_residues());
...
(Irrigation_operation   &irrig_op)
{
   ...
   CORN::Date_time_clad_64 op_start_datetime                                     //170425
      (today.get_date32(),(Seconds)irrig_op.get_start_hour()*3600);              //170425
   ...
   {                                                                             //060725
      ...
      {                                                                          //081202
         ...
         {  Chemical_profile *NH4_profile = dynamic_cast<Chemical_profile *>     //170425
...
{  Event_status_indicator event_status = ES_FAIL;
   Soil_chemicals_profile       *soil_chemicals = get_soil_chemicals();          //170630
   if (soil_chemicals
...
{  float64 auto_N_appl = 0.0; // kg/m2
   ...
   /*
   std::clog << "auto_Nappl = " << auto_N_appl
             << "\ntarget_yield_N_uptake_kg_ha = " << target_yield_N_uptake_kg_ha 
             << "\ntoal_automatic_N_application = " << total_automatic_N_application << std::endl;
   */
...
//_2012-07-10__2002-03-14__________perform_start_automatic_N_application_mode__/
#endif
#endif
   ...
   if (associated_crop_param)
   {
      ...
      {  //for each entry in current section:
         ...
         {  // lookup field in actual fields:
            //180820 std::string key(entry->get_key());
