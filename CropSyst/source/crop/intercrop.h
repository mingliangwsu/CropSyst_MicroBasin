...
: public implements_ Crop_interfaced
{
 ...
 public: // Crop_interfaced implementations
   inline virtual void *get_association_for_events()                       const { assert(false); return 0; /* This will never get called by intercrop*/}
...
*/
   ...
   //180413 renamed virtual const CORN::Date &get_largest_GAI_date()                       const;
   virtual const CORN::Date &ref_GAI_max_date()                           const;
   #ifdef CROP_ORCHARD
...
public:  // The following are processes used by the calling program
   ...
   virtual float64  update_pot_evapotranspiration(float64 pot_ref_evapotranspiration_m);
                                                                                 //140320 RLN need to check with Claudio.   virtual float64 est_degree_days_X_days_from_now
