...
nat32 Canopy_growth_portioned::vital_becomes_effete(bool apply_dormancy) modification_
{
   ...
   while (Canopy_accumulation::Portion *portion = (Canopy_accumulation::Portion *)mod_canopy_vital().pop_at_head())   //060911
   {
...
/*180718 moved to vital_becomes_effete
   while (Canopy_accumulation::Portion *portion = (Canopy_accumulation::Portion *)mod_canopy_vital().pop_at_head())   //060911
   {  if (apply_dormancy)                                                        //080401
         portion->dormancy();                                                    //060911
      mod_canopy_effete().append(portion);                                       //060911
...
{  // Note, could make the live_biomass a property of portion
 // and simply add this with the reset or update()
   ...
   {  FOR_EACH_IN(portion,const Canopy_accumulation::Portion ,get_canopy_vital() ,each_portion)
   ...
   FOR_EACH_IN(portion,Canopy_accumulation::Portion  ,get_canopy_effete() ,each_portion)
   {  if (!portion->is_dead()) // obs caller should make sure senescence does not apply duration already checked to determine if dead(then dont call this function) (parameters->has_leaf_senescence()  && (curr_daily->age > parameters->leaf_duration_deg_day_f32))
      live_biomass += portion->get_biomass_current();  // produced and current should be the same for live 060601
   } FOR_EACH_END(each_portion)
 return live_biomass;
...
{  return get_canopy_vital() .is_valid()
      ||  get_canopy_effete().is_valid()                                         //080807
...
{  // This sets the specified fraction of live biomass to effete
   ...
   FOR_EACH_IN(effete_portion,Canopy_accumulation::Portion,get_canopy_effete(),each_effete_portion)
   ...
   FOR_EACH_IN(vital_portion,Canopy_accumulation::Portion ,mod_canopy_vital(),each_vital_portion)
...
{  // This sets the specified fraction of live biomass to effete
   ...
   // Killing of effete first
   FOR_EACH_IN(effete_portion,Canopy_accumulation::Portion,mod_canopy_effete(),each_effete_portion)
   {  float64 living_this_portion = effete_portion->get_living();
      if ((remaining_to_die > 0.0)  &&
          (living_this_portion > 0.0))                                           //130626
         effete_portion->die();
...
{  float64 living_vital  = (include_living & include_vital)  ? get_canopy_vital().get_living() : 0.0;   //080808_060921
   float64 living_effete = (include_living & include_effete) ? get_canopy_effete().get_living(): 0.0;   //080808_060921
...
{  float64 biomass_current_vital = (include_biomass & include_vital) ?  get_canopy_vital().get_biomass_current()  : 0.0;        //080808_070321
   float64 biomass_current_effete =(include_biomass & include_effete) ? get_canopy_effete().get_biomass_current() : 0.0;        //080808_070321
...
{  float64 biomass_produced_vital = (include_biomass & include_vital) ? get_canopy_vital().get_biomass_produced() : 0.0;                        //070321
   float64 biomass_produced_effete =(include_biomass & include_effete) ?  get_canopy_effete().get_biomass_produced() : 0.0;                     //070321
      ...
      {  if (protect_it)                                                         //060317
         ...
         {  if (!get_untouchable_portion())                                      //080222
               ...
               float64 more_living_needed_for_regrowth
                  = regrowth_living - get_untouchable_portion()->get_living();   //080222
