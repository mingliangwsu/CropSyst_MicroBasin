...
bool Event_scheduler_crop::end_season()                                          modification_ //150930
{
   ...
   {
      //180615 Common_event  *event = dynamic_cast<Common_event  *>
...
{  Event_status_indicator status = ES_FAIL;
   ...
   if (associated_crop_param)
   {
      ...
      {  //for each entry in current section:
         ...
         //std::clog << " section->get_label():" << section->get_label() << std::endl;
...
///*180820
         ...
         {  // lookup field in actual fields:
            std::string key(entry->get_variable().c_str());
            //std::clog << " entry->get_variable():" << entry->get_variable().c_str() << std::endl;
