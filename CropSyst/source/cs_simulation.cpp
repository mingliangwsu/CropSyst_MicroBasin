...
//__________________________________________________________________destructor_/
#if (CS_VERSION == 4)
bool Cropping_system_simulation::Location_weather::is_key_string(const std::string &key) affirmation_  //180820
{
...
#else
bool Cropping_system_simulation::Location_weather::is_key_wstring(const std::wstring &key)              affirmation_  //180820
{
   assert(false); // check if this method is still used
...
{  // Lookup to_lbf in LBF_sims if not found return false
   ...
   {  const Geocoordinate &lbf_geocoord = existing_LBF->meteorology.get_geocoordinate(); //151208
      ...
      {  if (location_fname.has_extension_ASCII("loc",true))
         {  location_to_use = new Location_parameters("",true/*180626 ,CS_VERSION_NUMBERS*/);  // weather filename will be loaded
...
*/
            if (existing_LBF->scenario_control_and_model_options.substitute_year_base
               ...
               ,existing_LBF->scenario_control_and_model_options.start_date.get_year() // substitute reference year  //170331
...
//_2003-09-12___________________________________________________setup_location_/
const std::string &Cropping_system_simulation::get_model_name()            const
{ return CropSyst::model_name; };
} // namespace CropSyst_V4 //CS
//_2012-09-25___________________________________________________get_model_name_/
#endif
