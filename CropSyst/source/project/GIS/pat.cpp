...
{  // Extension will already be set in template (User does not have to use .SIL)
   ...
   CORN::OS::Directory_name_concrete fname_path(fname_for_path,CORN::OS::Directory_entry_name::include_path_designated);
...
//______________________________________________________________________________
/*161031 not needed
bool Polygon_attribute_data_record::get_end()
{}
*/
...
{
   data_rec.expect_int32 /*091203 expect_int16 */      (polygon_ID_field .c_str(),polygon_ID);
...
/*131008
   if (result == 0)   {  result = rotation_code.CORN::Ustring::compare(other.rotation_code);
   if (result == 0)   {  result = recalibration_code.CORN::Ustring::compare(other.recalibration_code);
   if (result == 0)   {  result = water_table_code.CORN::Ustring::compare(other.water_table_code);  //067060
   if (result == 0)   {  result = weather_code.CORN::Ustring::compare(other.weather_code);
   if (result == 0)   {  result = soil_code.CORN::Ustring::compare(other.soil_code);
*/
