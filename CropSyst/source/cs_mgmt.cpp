...
, bool run_nitrogen
, Synchronization_mode what_to_transfer                                          //151004
...
// original parameters (this is used only by the schedule viewer for cross referencing)
   ...
   #ifdef LADSS_MODE
   to_event_list.transfer_from(LADSS_operations              ,what_to_transfer); //151004_030810
   #endif
   to_event_list.transfer_from(tillage_operations            ,what_to_transfer); //151004_991122
      // Used only in LADSS mode, but may put all operations here in one list
   to_event_list.transfer_from(residue_operations            ,what_to_transfer); //151004_991122
   to_event_list.transfer_from(harvest_or_clipping_operations,what_to_transfer); //031014
   to_event_list.transfer_from(irrigation_operations         ,what_to_transfer); //151004_991122
   to_event_list.transfer_from(chemical_operations           ,what_to_transfer); //151004_991122
   to_event_list.transfer_from(auto_clip_biomass_mode_operations,what_to_transfer); //011208
   to_event_list.transfer_from(auto_irrigation_mode_operations,what_to_transfer);//151004_020313
#ifdef NITROGEN
   if (run_nitrogen)
   {  to_event_list.transfer_from(inorganic_nitrogen_operations,what_to_transfer);  //151004
#if ((CROPSYST_VERSION > 0) && (CROPSYST_VERSION < 5))
      to_event_list.transfer_from(organic_nitrogen_operations  ,what_to_transfer);  //151004
#endif
      to_event_list.transfer_from(biomatter_application_operations,what_to_transfer);//151004
      ...
      {  if (auto_N_applications.count())                                        //160328
         ...
         if ((N_application_soil_observation_event->begin_sync.get_sync_mode()   //160326
               == what_to_transfer) || (what_to_transfer == ANY_SYNC_MODE))      //160326
         {  to_event_list.append(N_application_soil_observation_event);          //060804
            N_application_soil_observation_event = 0;
      }  }
      to_event_list.transfer_from(auto_N_applications,what_to_transfer);         //151004_020314
   }
#endif
}
//______________________________________________________________________________
