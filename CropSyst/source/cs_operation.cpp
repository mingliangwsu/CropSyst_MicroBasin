...
const char *Operation::label_cstr_(char *result)                           const
...
{  Operation::label_string(buffer);
   ...
   buffer.append(var_code_labeled.get_label());
   return buffer.c_str();
}
...
{  Operation::setup_parameters(data_rec,for_write);
   data_rec.expect_bool(LABEL_terminate_crop,terminate_crop/*180327 ,CROPSYST_VV_BOOL_FORM*/);  //970616P  //110115 moved from tillage/residue/harvest
...
/*170426 replaced with label_string
const char *Field_operation::label_cstr_deprecated(char *result)                      const
{  // We dont call Common_operation::label because it is pure
   Operation::label_cstr_deprecated(result);                                                //020507
...
//_2001-12-09_____________________________________________________label_string_/
...
{}
//_____________________________________________________________________________/
void Field_operation_V4::setup_parameters
...
const char *Field_operation_V4::label_string(std::string &result)          const
{  // We dont call Common_operation::label because it is pure
   Field_operation::label_string(result) ;
   if (SCS_code)
   {
      result += " (SCS code:";
      result.append(lookup_SCS_description_deprecated_V4_7(SCS_code));
      result += ")";
   }
   result += ":";
   return result.c_str();
