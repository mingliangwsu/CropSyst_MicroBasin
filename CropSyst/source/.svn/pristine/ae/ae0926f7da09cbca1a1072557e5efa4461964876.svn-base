...
//   #endif
   ...
   ,const Soil_structure_interface            &soil_structure_                   //150925
   ,Infiltration_model                 infiltration_model_                       //080117
   ,Soil_abiotic_environment_profile   *abiotic_environment_profile_
   ,NO3_Profile                        &NO3_
...
,Soil_abiotic_environment_profile *abiotic_environment_profile_                  //060510
,Water_table_curve   *water_table_curve_)                                        //000504
:NO3_Profile
   (today_
   #if (CROPSYST_VERSION==4)
...
//      float64 CO2_C_loss_per_unit_soil_mass_daily_kgC_ha = per_m2_to_per_ha(CO2_C_loss_subsurface[layer]) ; //081010
     ...
     respiration_function
      ...
      bool exhausted = CORN::is_approximately<float64>(nitrate_N_mass,0.0);
...
// Temporarily overriding with the original value 1.0 until confirm with Claudio.
            ...
            float64 transformed_M_hour_pot                                       //170503RLN
               = element_to_chem_factor*total_denitrification_kgN_m2_h_pot;      //170503RLN
            float64 transformed_M_hour_act                                       //170503RLN
               = mass->dec_layer(layer,transformed_M_hour_pot);                  //170503RLN
            exhausted = CORN::is_approximately
               (transformed_M_hour_act,0.0,0.0000000001);                        //170503RLN
            float64 denitrification_act_kgN_m2_h                                 //170503RLN
               = chem_to_element_factor * transformed_M_hour_act;                //170503RLN
...
#ifdef DETAIL_DENITR
//if (CORN::is_approximately<float64>(total_denitrification_kgN_m2_h,0.0, 0.000000001)) // 170214
//if (simulation_today.get()==1860004)
//(std::clog)<< (int) simulation_today.get() << '\t'
if (denitr_detail_stream)
(*denitr_detail_stream)<< (int) /*170626 simulation_today*/ today.get_date32() << '\t'            //170525
<< (int)layer << '\t'
<< interval << '\t'
<< nitrate_N_mass*10000.0 << '\t'
<< denitrification_act_kgN_m2_h /*170503 total_denitrification_kgN_m2_h*/ << '\t'
<< N2O_N_loss_interval << '\t'
...
*/
      ...
      float64 transformed_M = element_to_chem_factor*N_denitrified_daily_layer_output_bal;
      /*170503 deducted hourly
      mass->Xdec_layer(layer,transformed_M);
      */
      ...
      #else
      if (receiver) // NYN
      receiver->take_elemental_from_transformation(layer,N_denitrified_daily_layer_output_bal); //170502
      #endif
...
{
   //170217unused static const float64 nitrification_constant_daily  = 0.05; //160223 was 0.2   10.2; // 1/day            //060510
   ...
   {  // Do not use the Soil_abiotic_environment moisture function used by the organic matter decomposition.
      ...
      bool exhausted = CORN::is_approximately<float64>(ammonium_N_mass,0.0);     //170503
      for (Hour hour = 0; (hour < 24) && !exhausted; hour ++)                    //170503
      {  // RLN warning this is using the N masses from the beginning of the day.
         //171503 checking exhausted if (ammonium_N_mass > 0)
         {  float64 nitrification_moisture_function =  calc_moisture_function    //070117
            ...
            */
            float64 N_oxidized_hour_M_act = mass->dec_layer
               (layer,N_oxidized_hour_pot*element_to_chem_factor);
            float64 N_oxidized_hour_act
               = chem_to_element_factor * N_oxidized_hour_M_act;
      ...
      inc_transformed_to(transformed_M);                                         //990317
      //170503 mass->Xdec_layer(layer,N_oxidized_total_day * element_to_chem_factor);      //170502_080116
