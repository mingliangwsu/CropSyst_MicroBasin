...
Soil_parameters::~Soil_parameters()
{}
//_1998-07-22__________________________________________________________________/
...
#ifdef VERSION3
   expect_bool          (LABEL_override_leaching_depth,override_leaching_depth,CROPSYST_VV_BOOL_FORM); //020324
...
//_1998-12-12__________________________________________________________________/
...
//161023 moved to end Common_parameters_data_record::get_end();                                     //051028
   ...
   if (!CORN::is_approximately<float32>(old_pH_32,0.0,0.0001))
      for (nat8 layer = 1; layer <= number_horizons; layer++)
          if (CORN::is_approximately<float32>(horizon_pH[layer],DEF_pH,0.00001)
          || (CORN::is_approximately<float32>(horizon_pH[layer],0.0,0.00001)))
            horizon_pH[layer] = old_pH_32;
   if (!CORN::is_approximately<float32>(old_cation_exchange_capacity_32,0.0,0.0001))
      for (nat8 layer = 1; layer <= number_horizons; layer++)
          if (CORN::is_approximately<float32>
           ...
           || CORN::is_approximately<float32>
               (horizon_cation_exchange_capacity[layer],0.0,0.00001))
            horizon_cation_exchange_capacity[layer] = old_cation_exchange_capacity_32;
   return Soil_parameters_class::get_end();                                      //161023
}
//_2001-11-30__2001-01-09______________________________________________________/
#ifdef VERSION3
...
//_1998-11-04__________________________________________________________________/
#endif
